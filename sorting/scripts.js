...
let numBars = 100;
let barWidthUnit = width * (1 / numBars);
let barHeightUnit = height * (1 / numBars);
...
function updateBars() {
    ...
    // Init max and min number of bars
    min = 10
    max = 1000

    val = document.getElementById("numBarsInput").value;

    if (val > max) {
        val = max;
        document.getElementById("numBarsInput").value = max;
    }
    else if (val < min) {
        val = min;
        document.getElementById("numBarsInput").value = min;
    }

    // Update the number of bars and their units
    numBars = val;
    barWidthUnit = width * (1 / numBars);
    barHeightUnit = height * (1 / numBars);

    // Refresh the bars to avoid any weirdness
    Screen1.newBars();
    Screen1.drawBars();
